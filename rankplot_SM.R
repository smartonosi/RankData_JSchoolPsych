rankplot_SM <- function (dset, trans = FALSE, label.type = "item", circle.col = "black", 
          circle.bg = FALSE) 
{
  nitem <- ncol(dset) - 1
  ...
  if (nitem == 3) {
    ...
    for (i in (1:6)) {
      for (j in (1:6)) {
        if (test2[i, 1] * 100 + test2[i, 2] * 10 + test2[i, 
                                                         3] == label[j]) {
          freq[j] <- test2[i, 4]
        }
      }
    ...
    graphics::plot(x, y, axes = FALSE, ann = FALSE, xaxt = "n", 
                   yaxt = "n", xlim = c(-1.482, 2.982), ylim = c(-2.232, 
                                                                 2.232))
    graphics::symbols(x, y, circles = 1.2*freq^0.5/sum(freq^0.5)/2, 
                      inches = FALSE, add = TRUE, fg = circle.col, bg = circle.bg)
  ...
  if (nitem == 4) {
    ...
    freq <- rep(0, 24)
    for (i in (1:24)) {
      label[i] <- test2[i, 1] * 1000 + test2[i, 2] * 100 + 
        test2[i, 3] * 10 + test2[i, 4]
    }
    for (i in (1:24)) {
      for (j in (1:24)) {
        if (test2[i, 1] * 1000 + test2[i, 2] * 100 + 
            test2[i, 3] * 10 + test2[i, 4] == label[j]) {
          freq[j] <- test2[i, 5]
        }
      }
    }
    ...
    if (trans == FALSE) {
      x <- c(0.76, 1.26, 0.44, 1.51, 0.64, 1.24, 1.15, 
             1.61, 0.55, 2.2, 0.77, 1.98, 1.27, 2.23, 1, 2.55, 
             1.53, 2.15, 1.88, 2.38, 1.66, 2.72, 1.98, 2.56)
      y <- c(-1.02, -0.56, -1.22, -0.24, -0.95, -0.4, -1.43, 
             -1, -1.93, -0.4, -1.7, -0.58, -2.04, -1.15, -2.33, 
             -0.88, -1.93, -1.36, -2.23, -1.77, -2.55, -1.55, 
             -2.37, -1.83)
    }
    else {
      x <- c(1, 1.66, 0.55, 1.98, 0.77, 1.53, 1.27, 1.88, 
             0.44, 2.56, 0.64, 2.15, 1.15, 2.38, 0.76, 2.72, 
             1.24, 1.98, 1.61, 2.23, 1.26, 2.55, 1.51, 2.2)
      y <- c(-2.33, -2.55, -1.93, -2.37, -1.7, -1.93, -2.04, 
             -2.23, -1.22, -1.83, -0.95, -1.36, -1.43, -1.77, 
             -1.02, -1.55, -0.4, -0.58, -1, -1.15, -0.56, 
             -0.88, -0.24, -0.4)
    }
    graphics::symbols(x, y, circles = 1.2*freq^0.5/sum(freq^0.5)/2, 
                      inches = FALSE, add = TRUE, fg = circle.col, bg = circle.bg)
